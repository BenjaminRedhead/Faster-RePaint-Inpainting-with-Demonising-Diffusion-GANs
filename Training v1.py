...
def maskgen(): #Implements the mask built by TK as a function -BR
...
def sample_posterior(coefficients, x_0,x_t, t, x_i):
    ...
    def q_posterior(x_0, x_t, t, x_i):
        mx_0 = implement_masking(x_0, x_i)
        mean = (
    ...
    sample_x_pos = p_sample(x_0, x_t, t, x_i)
    
    return sample_x_pos

def sample_from_model(coefficients, generator, n_time, x_init, T, opt, x_i):
    #added the original image as a new parameter -BR
    x = x_init
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            x_new = sample_posterior(coefficients, x_0, x, t, x_i)
            x = x_new.detach()
...
def train(rank, gpu, args):
    ...
    if args.dataset == 'cifar10':
        ...
        #first data set to test on for repainting
        train_transform = transforms.Compose([
                transforms.Resize(args.image_size),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            x_pos_sample = sample_posterior(pos_coeff, x_0_predict, x_tp1, t, real_data) #Includes new parameters needed - BR
        ...
        if rank == 0:
            ...
            fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args, real_data) #Includes new inputs needed -BR
