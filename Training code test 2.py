...
def sample_posterior(coefficients, x_0,x_t, t, x_i, m):
    ...
    def implement_masking(x_0,x_i,m):
        # this function is implementing the masking -BR
        mask = cv.imread(m)
        inv = ImageOps.invert(mask)
        mx_0 = (x_0 - inv) + (x_i - mask)
        return mx_0
    
    def q_posterior(x_0, x_t, t, x_i, m):
        mx_0 = implement_masking(x_0, x_i, m)
        mean = (
            extract(coefficients.posterior_mean_coef1, t, x_t.shape) * mx_0
    ...
    def p_sample(x_0, x_t, t, x_i, m):
        mx_0 = implement_masking(x_0, x_i, m)

        mean, _, log_var = q_posterior(mx_0, x_t, t)
    ...
    sample_x_pos = p_sample(x_0, x_t, t, x_i, m)
    
    return sample_x_pos

def sample_from_model(coefficients, generator, n_time, x_init, T, opt, x_i , m):
    #added the original image and the mask as new parameters -BR
    x = x_init
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            x_new = sample_posterior(coefficients, x_0, x, t, x_i, m)
            x = x_new.detach()
...
def train(rank, gpu, args):
    ...
    if args.dataset == 'cifar10':
        ...
        #first data set to test on for repainting, we need to alter this to have two parameters, the original image and a mask, look at the Nvidia original inpainting...-BR
        #... with GANs for a dataset with masks - BR
        train_transform = transforms.Compose([
                transforms.Resize(args.image_size),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            #can someone check if this is inputting the images and how to alter for data with original image and mask to just use the image for this part? -BR
            ...
            x_pos_sample = sample_posterior(pos_coeff, x_0_predict, x_tp1, t) #still need to change to include original image and mask - BR
            ...
            x_0_predict = netG(x_tp1.detach(), t, latent_z)
            x_pos_sample = sample_posterior(pos_coeff, x_0_predict, x_tp1, t) #also need to change for original image and mask - BR
        ...
        if rank == 0:
            ...
            fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args) #need to change to include original image and mask -BR
